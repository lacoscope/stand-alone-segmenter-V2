...
# Libraries manipulate json format, manipulate paths
import json, os
...
class SegmenterProcess():
    ...
    def _extract_metadata_from_regionprop(self, prop):
        ...
        return {
            ...
            "area_exc": prop.area * (pixel_size**2),
            # Surface area of the object in square pixels
            "area": prop.filled_area * (pixel_size**2),
            ...
            "bounding_box_area": prop.bbox_area * pixel_size,
            "eccentricity": prop.eccentricity * pixel_size,
            "equivalent_diameter": prop.equivalent_diameter * pixel_size,
            "euler_number": prop.euler_number,
            "extent": prop.extent * pixel_size,
            "local_centroid_col": prop.local_centroid[1] * pixel_size,
            "local_centroid_row": prop.local_centroid[0] * pixel_size,
...
if __name__ == "__main__":
    ...
    data_path=filedialog.askdirectory(parent=root)
    root.destroy()
    segmenter=SegmenterProcess(data_path)
    sucess=segmenter.segment_path(data_path, True)
    root = tk.Tk()
    root.withdraw()
    root.call('wm', 'attributes', '.', '-topmost', True)
    if sucess==True:
        tk.messagebox.showinfo("SUCCESS ! ",message= "Segmentation succesfully completed", parent=root)
    else: 
        tk.messagebox.showerror("ERROR ! ", message= "An error occured during segmentation, try with local_segementer.py to debug.", parent=root)
